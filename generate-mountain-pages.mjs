...
function matcherScript(resort) {
  const pl = passLabel(resort.passGroup);
  const tb = resort.terrainBreakdown;
  const advPct = Math.round(tb.advanced * 100);
  const begPct = Math.round(tb.beginner * 100);
  const resortPass = resort.passGroup; // 'Epic', 'Ikon', 'Indy', 'Independent'

  // Build outcome logic as a JS string embedded in the page
  return `
<script>
(function() {
  ...
  document.querySelectorAll('.mo').forEach(function(btn) {
    btn.addEventListener('click', function() {
      var g = btn.dataset.g;
      document.querySelectorAll('.mo[data-g="' + g + '"]').forEach(function(b) { b.classList.remove('selected'); });
  ...
  function evaluate() {
    ...
    var resortPass = '${esc(resortPass)}';
    var advPct = ${advPct};
    var begPct = ${begPct};
    var price = ${resort.price};
    var name = '${esc(resort.name)}';

    var passMatches = (pass === resortPass.toLowerCase());

    if (skill === 'advanced') {
      if (passMatches) {
        type = 'yes';
        verdict = '✓ ' + name + ' is an excellent match for you.';
        ...
        reason = 'The mountain suits your skill level perfectly. Just budget accordingly — window rates can add up fast for a multi-day trip.';
        ctaText = 'Find the best value mountain for your level →';
      } else {
        type = 'maybe';
        verdict = '→ Great terrain, but your pass does not cover ' + name + '.';
        reason = 'You\'d be paying window rate ($' + price + '+). Check if a nearby mountain on your pass offers comparable terrain.';
      ...
      if (passMatches) {
        ...
        reason = 'Your pass removes the cost barrier and the mountain has plenty of intermediate terrain to explore. Midweek visits are best for avoiding crowds.';
        ...
        reason = 'Your pass does not cover ' + name + ', so you\'d pay window rate. A nearby mountain on your pass might offer a comparable experience for free.';
      ...
      if (begPct >= 35) {
        ...
        verdict = passMatches
          ? '✓ ' + name + ' has a good beginner setup and your pass covers it.'
          : '→ Decent beginner terrain, but you\'ll pay window rate.';
        reason = begPct + '% of the mountain is beginner terrain. ' + (passMatches ? 'Your pass makes this a low-risk day out.' : 'At $' + price + '+, it\'s worth comparing nearby options on your pass first.');
        ...
        reason = 'Only ' + begPct + '% of the mountain is beginner terrain and it skews toward more advanced skiers. You\'d spend the day navigating terrain that\'s above your comfort level.';
    ...
    document.getElementById('mCTA').href = 'https://wheretoskinext.com/?resort=${esc(resort.id)}';
  }
})();
</script>`;
}
