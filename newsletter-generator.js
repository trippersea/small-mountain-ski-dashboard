...
module.exports = async function handler(req, res) {
  ...
  // Timing-safe comparison (see _security.safeEqual) so the secret can't be
  // probed byte-by-byte through response timing.
  const { safeEqual } = require('./_security');
  const isAuthorized = authHeader.startsWith('Bearer ')
    && safeEqual(authHeader.slice('Bearer '.length), cronSecret);
